...
const POLL_DELAY = 100; // I forget which units this is :/
...
class TerminationPoller {
  ...
  async poll(watchdog, sharedState) {
    ...
    if (toPoll.length && toPoll.length === 0) {
      return;
    }
    ...
    await Promise.all(this.regions.map(async region => {
      ...
      // Let's find the instance ids for our region
      let instanceIds = toPoll.filter(x => x.region === region).map(x => x.id);
      log.info({instanceIds, region}, 'polling terminations');

      // We don't want to bother with the rest of this function if there's no
      // instance ids in this region
      ...
      try {
        ...
        log.warn({err}, 'failed to describe instances in api');
        throw err;
        return;
      }
