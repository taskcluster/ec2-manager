      ...
      for (let reservation of instances.Reservations) {
        for (let instance of reservation.Instances) {
          if (instance.LaunchTime) {
            ...
            // If we don't own this instance, we just skip it outright
            if (!managerId) {
            ...
            // But if it has a managerId and no workerType, then we know it's
            // bad and needs to be terminated
            if (!workerType) {
              log.error({instanceId: id}, 'missing Name tag');
              zombieIds.push(id);
              continue;
            }
