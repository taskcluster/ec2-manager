...
class HouseKeeper {
  ...
  async _submitVolumeTotals(totalsByCategory) {
    ...
    // We will add data to the database table in a single update, so store
    // the individual row data in an array.
    let rowData = [];
    ...
    if (rowData.length > 0) {
      // Submit row data to ebsusage table
      let transaction = await this.state.beginTransaction();
      try {
        await this.state.reportEbsUsage(rowData); //, transaction);
