...
class State {
  ...
  /**
   * Begin a transaction and return the client which should be used for all
   * database operations
   */
  async beginTx() {
  ...
  /**
   * Get a client based on one passed in or it that's falsy, create a new one
   */
  async _getClient(client) {
    if (client) {
      return client;
    } else {
   ...
   */
  async upsertSpotRequest({workerType, region, instanceType, id, state, status}, client) {
  ...
  async updateInstanceState({region, id, state}, client) {
    ...
    client = await this._getClient(client);
   ...
   * been fulfilled
   */
  async insertInstance({workerType, region, id, instanceType, state, srid}, client) {
    assert(typeof workerType === 'string');
    assert(typeof region === 'string');
    assert(typeof instanceType === 'string');
    assert(typeof id === 'string');
   ...
   */
  async upsertInstance({workerType, region, instanceType, id, state, srid}, client) {
    assert(typeof workerType === 'string');
    assert(typeof region === 'string');
    assert(typeof instanceType === 'string');
    assert(typeof id === 'string');
      ...
      for (let condition in conditions) {
        if (Array.isArray(conditions[condition])) {
          ...
          for (let value of conditions[condition]) {
            conditionClauses.push(`${condition} = $${paramIndex}`);
            values.push(value);
            paramIndex++;
          }
  ...
  async spotRequestsToPoll({region}, client) {
    let result = await this.listSpotRequests({region}, client);
