...
class State {
  ...
  async removeInstance({region, id, srid}, client) {
   ...
   /**
   * Insert an AMI's first usage.
   */
  async insertAmiUsage({region, id}, client) {
    assert(typeof region === 'string');
    assert(typeof id === 'string');
    ...
    assert(result.rowCount === 1, 'inserting AMI usage had incorrect rowCount');
  }
   
   /**
   * Either insert or update an AMI's usage.  This ensures an atomic insert or
   * update, but we never learn which one happened. Only updates the row 
   * if the proposed time is newer than the existing time.
   */
  async upsertAmiUsage({region, id}, client) {
    assert(typeof region === 'string');
    assert(typeof id === 'string');
    
    let text = [
      "INSERT INTO amiusage",
      "(region, id, lastused)",
      "VALUES ($1, $2, CURRENT_TIMESTAMP)",
      "ON CONFLICT (region, id ) DO UPDATE SET lastused = CURRENT_TIMESTAMP"
    ].join(' ');
   ...
   /**
   * Update an AMI's usage.
   */
  async updateAmiUsage({region, id}, client) {
    assert(typeof region === 'string');
    assert(typeof id === 'string');

    let text = "UPDATE amiusage SET lastused = CURRENT_TIMESTAMP WHERE region = $1 AND id = $2 AND lastused < CURRENT_TIMESTAMP";
    let values = [region, id];
    let name = 'update-amiusage';
    ...
    assert(result.rowCount === 1, 'updating AMI usage had incorrect rowCount');
  }
   
   async reportAmiUsage(region, id) {
     this.upsertAmiUsage({region: region, id: id});
   }
