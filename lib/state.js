...
const TERMINATION_CODES = {
  cleanShutdown: ['Client.InstanceInitiatedShutdown', 'Client.UserInitiatedShutdown'],
  spotKill: ['Server.SpotInstanceTermination'],
  insufficientCapacity: ['Server.InsufficientInstanceCapacity'],
  volumeLimitExceeded: ['Client.VolumeLimitExceeded'],
  missingAmi: ['Client.InvalidSnapshot.NotFound'],
  startupFailed: ['Server.InternalError', 'Client.InternalError'],
};
  ...
  */
  ...
  async getRequestHealth({end, start, workerType}) {
    ...
    let query = [
      ...
      'count(nullif(code LIKE \'Invalid%\', FALSE)) AS "configurationIssue",',
      'count(nullif(code = \'RequestLimitExceeded\', FALSE)) AS "throttledCalls",',
      'count(nullif(code LIKE \'Insufficient%\', FALSE)) AS "insufficientCapacity",',
      'count(nullif(code LIKE \'%LimitExceeded\' AND code != \'RequestLimitExceeded\', FALSE)) as "limitExceeded"',
      'FROM awsrequests',
  ...
  async getTerminationHealth({end, start, workerType}) {
    ...
    query.push(`, count(nullif(code = \'${knownCodes.join('\' OR code = \'')}\', TRUE)) AS "unknownCodes"`);

    // We also want to show how many terminations had no code, meaning they weren't found
    // in the database
    query.push(', count(nullif(code is null, FALSE)) as "noCode"');
  ...
  async getRecentErrors(opts) {
    ...
    let query = [
      ...
      'FROM terminations',
      `WHERE code <> \'${TERMINATION_CODES.cleanShutdown.join('\' AND code <> \'')}\'`,
