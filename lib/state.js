...
class State {
  ...
  async updateTerminationState({region, id, code, reason, terminated, lastEvent}, client) {
    ...
    assert(typeof lastEvent === 'object', 'incorrect lastEvet type');
    assert(lastEvent.constructor.name === 'Date', 'lastEvent is not a Date');

    let text = 'UPDATE terminations SET code = $1, reason = $2, terminated = $3, "lastEvent" = $4';
    text += ' WHERE region = $5 AND id = $6';
    let values = [code, reason, terminated, lastEvent, region, id];
    let name = 'update-termination-state';
  ...
  async listTerminations(conditions = {}, client = undefined) {
    assert(typeof conditions === 'object');
    return this._listTable('terminations', conditions, client);
  }
