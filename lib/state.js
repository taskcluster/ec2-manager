...
function assertValidInstance (instance) {
  ...
  for (let key of requiredKeys) {
    if (!instance[key]) {
      ...
      if (typeof instance[key] !== 'object') {
        errors.push(`Key "${key}" is incorrect type "${typeof instance[key]}"`);
      }
  ...
  for (let key of allowedKeys) {
    if (instance[key] && typeof instance[key] !== 'string') {
      errors.push(`Optional key "${key}" is incorrect type "${typeof instance[key]}"`);
    }
  }
...
class State {
  ...
  async upsertSpotRequest(spotRequest, client) {
    ...
    let text = [
      "INSERT INTO spotrequests (id, workerType, region, az, instanceType, state, status, imageid, created)",
      "VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)",
      "ON CONFLICT (region, id) DO UPDATE SET state = $6, status = $7;"
    ].join(' ');
  ...
  async insertInstance(instance, client) {
    ...
    try {
      ...
      try {
        let text = [
          "INSERT INTO instances (id, workerType, region, az, instanceType, state, srid, imageid, launched)",
          "VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)"
        ].join(' ');

        let values = [id, workerType, region, az, instanceType, state, srid ? srid : 'NULL', imageId, launched];
    ...
    try {
      ...
      try {
        let text = [
          "INSERT INTO instances (id, workerType, region, az, instanceType, state, srid, imageid, launched)",
          "VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)",
          "ON CONFLICT (region, id) DO UPDATE SET state = $6;"
        ].join(' ');
        let values = [id, workerType, region, az, instanceType, state, srid ? srid : 'NULL', imageId, launched];
