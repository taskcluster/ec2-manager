...
class State {
  ...
  */
  ...
  async getHealth(opts) {
    ...
    let end = (await pool.query('SELECT now();')).rows[0].now;
    ...
    // TODO: use LIKE "Invalid%" for configuration_errors
    ...
    //Store the value for the response
    response.requestHealth = result.rows || [];
    log.debug(response.requestHealth, 'determined request health');

    // Now we're going to grade the terminations for each configuration.  This
    ...
    response.terminationHealth = result.rows || [];
    log.debug(response.terminationHealth, 'determined termination health');

    // Now we'll list the running instances.  This is not filtered by the
    // number of minutes, since everything which is currently running is...
    // running and should be included in the statistics
    query = [
    ...
    response.running = result.rows || [];
    log.debug(response.running, 'determined running stats');
