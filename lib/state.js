...
class State {
  ...
  */
   ...
   * specified
   */
  async getRequestHealth({end, start}) {
   ...
   */
  async getTerminationHealth({end, start}) {
    assert(typeof end === 'object');
    assert(typeof start === 'object');
    assert(end.constructor.name === 'Date');
    assert(start.constructor.name === 'Date');
  ...
  async getRunningInformation() {
    ...
    let result = await this._pgpool.query(query);
  ...
  async getHealth(opts) {
    ...
    return {
      requestHealth: await this.getRequestHealth({start, end}),
      terminationHealth: await this.getTerminationHealth({start, end}),
      running: await this.getRunningInformation(),
    };
  }
