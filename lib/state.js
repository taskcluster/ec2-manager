...
class State {
  ...
  async logCloudWatchEvent({region, id, state, received}, client) {
    assert(typeof region === 'string');
    assert(typeof id === 'string');
    assert(typeof state === 'string');
    assert(typeof received === 'object');
    assert(received.constructor.name === 'Date');

    let text = [
      "INSERT INTO cloudwatchlog (id, region, state, received)",
      "VALUES ($1, $2, $3, $4);"
    ].join(' ');
    let values = [id, region, state, received];
