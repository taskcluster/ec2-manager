...
class State {
  ...
  async removeInstance({region, id, srid}, client) {
   ...
   /**
   * Either insert or update AMI usage.  This ensures an atomic insert or
  ...
  async upsertAmiUsage({region, id}, client) {
    ...
    let text = [
      "INSERT INTO amiusage (id, region)",
      "VALUES ($1, $2)",
      "ON CONFLICT (region, id) DO UPDATE amiusage SET lastused = CURRENT_TIMESTAMP WHERE lastused < CURRENT_TIMESTAMP;"
    ].join(' ');
    ...
    assert(result.rowCount === 1, 'upserting amiusage had incorrect rowCount');
  }
   
   async reportAmiUsage(region, id) {
      upsertAmiUsage({region, id});
   }
