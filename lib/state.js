...
class State {
  ...
  */
  ...
  async reportEbsUsage(rowData) { //, client) {
    ...
    // tuple, so first clear the table.
    await this.clearEbsUsage(transaction);

    let values = rowData.map(row => {
      let {region, volumetype, state, totalcount, totalgb} = row;
      assert(typeof region === 'string');
      assert(typeof volumetype === 'string');
      assert(typeof state === 'string');
      assert(typeof totalcount === 'number');
      assert(typeof totalgb === 'number');
      return `('${region}', '${volumetype}', '${state}', ${totalcount}, ${totalgb})`;
    }).join(', ');
    let text = [
      'INSERT INTO ebsusage (region, volumetype, state, totalcount, totalgb) VALUES ',
      ';',
    ].join(values);
    let name = 'report-ebs-usage';

    let result = await transaction.query(text);
    await this.commitTransaction(transaction);
    assert(result.rowCount === rowData.length, 'inserting EBS usage had incorrect rowCount');
  }

  async clearEbsUsage(client) {
    let text = 'TRUNCATE TABLE ebsusage;';
    
    if (client) {
      await client.query(text);
    } else {
      // We should never arrive in this case, since we clear the database and insert
      // new values in the same transaction.
      throw new Error('Client should not be undefined in clearEbsUsage');
    }
  }
