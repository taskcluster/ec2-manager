...
class State {
  ...
  */
  ...
  async gradeConfigurations({minutes}) {
    minutes = minutes || 2 * 60;
    assert(typeof minutes === 'number', 'minutes must be a number');

    let pool = this._pgpool;

    // Rather than doing locking on the DB, which isn't even possible
    // for queries which have aggregating functions, we'll instead timebox
    // our queries on the 'called' parameter.  Since we're never editing
    // things in the tables, we're pretty safe to do this.
    // Instead of using our time, we're going to use the DB server's
    // time
    let start = (await pool.query('SELECT now();')).rows[0].now;
    ...
    // Let's get the health of each configuration for requests to runInstances
    query = [
    ...
    let requestHealth = result.map(config => {
      return {
        region: config.region,
        az: config.az,
        instanceType: config.instanceType,
        count: config.successful + config.failed,
        failureRate: config.failed / (config.successful + config.failed);
      };
    });

    // Let's get the health of instance shutdowns


  }

  async getHealth({minutes}) {
    minutes = minutes || 2 * 60;
    assert(typeof minutes === 'number', 'minutes must be a number');
    ...
    // Let's have a summary of the calls we've made
    query = [
      'SELECT region, method, count(region) as calls, count(nullif(error=true, false)) as errors',
      'FROM awsrequests',
      'WHERE called > $1 AND called <= $2',
      'GROUP BY region, error, method;',
    ].join(' ');
    result = await pool.query({
      text: query,
      values: [start, end],
    });
    health.callSummary = result.rows;

    // Let's have a summary of the error codes that we're seeing
    query = [
      'SELECT region, method, code, count(code) from awsrequests',
      'WHERE error = true AND called > $1 AND called <= $2',
      GROUP BY region, method, code;
    ].join(' ');
    result = await pool.query({
      text: query,
      values: [start, end],
    });
    health.errorSummary = result.rows;
