...
class State {
   ...
   */
  async upsertSpotRequest({workerType, region, instanceType, id, state}) {
    let text = "INSERT INTO spotrequests (id, workerType, region, instanceType, state) ";
    text += "VALUES ($1, $2, $3, $4, $5) ";
    text += "ON CONFLICT (region, id) DO UPDATE SET state = $6;";
    let values = [id, workerType, region, instanceType, state, state];
    let result = await this._pgpool.query({text, values});
  ...
  async upsertInstance({workerType, region, instanceType, id, state, srid}) {
    ...
    try {
      ...
      try {
        let text = "INSERT INTO instances (id, workerType, region, instanceType, state, srid) ";
        text += "VALUES ($1, $2, $3, $4, $5, $6) ";
        text += "ON CONFLICT (region, id) DO UPDATE SET state = $5;";
