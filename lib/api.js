...
/**
 * Make a request for an on demand instance
 */
api.declare({
  method: 'put',
  route: '/worker-types/:workerType/on-demand',
  name: 'runInstance',
  title: 'Request an on-demand instance',
  stability: API.stability.experimental,
  input: 'on-demand-request.json#',
  scopes: [['ec2-manager:manage-resources:<workerType>']],
  description: [
    'Request an on-demand instance of a worker type'
  ].join(' '),
}, async function (req, res) {
  try {
  } catch (err) {
