...
api.declare({
  ...
  stability: API.stability.experimental,
  scopes: [['ec2-manager:request-spot-instance:<workerType>']],
  description: [
  ...
  let valid = await this.lsChecker.check({
    launchSpecificaion: LaunchSpecification,
    region: Region,
  });
...
api.declare({
  ...
  await Promise.all(this.regions.map(async region => {
    ...
    // POSSIBLE OPTIMIZATION: add a remote{Instance,SpotRequest}s method to
    // bulk remove them
