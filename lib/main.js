...
const {ZombieKiller} = require('./zombie-killer');
...
let load = loader({
  ...
  zombiekiller: {
    ...
    setup: async ({cfg, state, ec2, monitor, runaws}) => {
      ...
      return new Iterate({
        ...
        watchDog: 1000 * 60 * 15,
        maxFailures: 10,
        waitTime: cfg.app.zombiePollDelay,
        handler: async (watchdog) => {
          try {
            await zk.kill();
          } catch (err) {
            console.dir(err);
            monitor.reportError(err, 'Error running zombie killer');
          }
        },
      });
    }
  ...
  start: {
    requires: ['eventlisteners', 'deadeventlisteners', 'server', 'spotpollers', 'zombiekiller'],
    setup: async ({eventlisteners, deadeventlisteners, server, spotpollers, zombiekiller}) => {
