...
let load = loader({
  ...
  api: {
    requires: ['cfg', 'validator', 'monitor', 'state'],
    setup: async ({cfg, validator, monitor, state}) => {
  ...
  spotpollers: {
    requires: ['cfg', 'state', 'ec2'],
    setup: async ({cfg, state, ec2}) => {
  ...
  start: {
    requires: ['eventlisteners', 'server', 'spotpollers'],
    setup: async ({eventlisteners, server, spotpollers}) => {
