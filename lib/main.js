...
let load = loader({
  ...
  startlisteners: {
    requires: ['eventlisteners', 'deadeventlisteners'],
    setup: async({eventlisteners, deadeventlisteners}) => {
      eventlisteners.map(x => x.start());
      deadeventlisteners.map(x => x.start());
      log.info('Started listeners');
    },
  },
  ...
  startspotpoller: {
    requires: ['spotrequestpoller'],
    setup: async({spotrequestpoller}) => {
      spotrequestpoller.start();
      log.info('Start spot request poller');
    },
  },
