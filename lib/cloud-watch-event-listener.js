...
class CloudWatchEventListener extends events.EventEmitter {
  ...
  async init() {
    ...
    this.sqsQueue.on('error', (err, errType) => {
      ...
      this.monitor.reportError(err, errType, 'SQS Handler Error');
    });
  }
  ...
  async __handler(msg) {
    ...
    if (state === 'pending' || state === 'running') {
      let apiResponse = await this.awsrun(this.ec2, 'describeInstances', {
        InstanceIds: [id]
      });
...
class DeadCloudWatchEventListener extends events.EventEmitter {
  ...
  async init() {
    ...
    this.sqsQueue.on('error', (err, errType) => {
      ...
      if (err.code !== 'InvalidInstanceID.NotFound') {
        this.monitor.reportError(err, {type: errType});
      }
    });
  }
  ...
  async __handler(msg) {
    ...
    this.monitor.reportError(new Error('Untracked Instance'), 'info');
  }
