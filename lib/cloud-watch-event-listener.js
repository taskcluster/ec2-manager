...
class CloudWatchEventListener extends events.EventEmitter {
  ...
  async __handler(msg) {
    ...
    // the database, we'll just blindly delete them and return early as there's
    ...
    if (dbResponse.length > 1) {
      ...
      assert(dbResponse.id === id, 'database id does not match');
      assert(dbResponse.region === region, 'database region does not match');
      ...
      // We don't want to use the same transaction because there's no need.  We
      // don't want to hold the lock longer than needed
      await this.state.commitTransaction(transaction);
