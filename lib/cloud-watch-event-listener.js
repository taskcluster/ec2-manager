...
class CloudWatchEventListener extends events.EventEmitter {
  ...
  async __handler(msg) {
    ...
    // We're logging this information to a database table so that we can
    // see when these events happen
    try {
      await this.state.logCloudWatchEvent({region, id, state, received: generated});
    } catch (err) {
    ...
    if (dbResponse) {
      ...
      if (dbResponse.lastevent < generated) {
        try {
