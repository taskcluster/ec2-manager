...
class Database extends EventEmitter{
  ...
  async listAllSpotRequests() {
    let result = await this._runQuery({text: "SELECT * FROM spotrequests"});
    return result.rows;
  }

  async listInstancesByWorkerType(workerType) {
    let query = "SELECT * FROM instances WHERE workerType = $1";
    return new Promise((resolve, reject) => {
      this._client.query(query, [workerType], (err, result) => {
        if (err) {
          return reject(err);
        }
        resolve(result.rows);
  ...
  async listSpotRequestByWorkerType(workerType) {
    let query = "SELECT * FROM spotrequests WHERE workerType = $1";
    return new Promise((resolve, reject) => {
      this._client.query(query, [workerType], (err, result) => {
        if (err) {
          return reject(err);
