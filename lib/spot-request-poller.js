...
class SpotRequestPoller {
  ...
  constructor({ec2, regions, runaws = runAWSRequest, spotPollDelay = 25, state, monitor}) {
    assert(typeof ec2 === 'object');
    assert(Array.isArray(regions));
    for (let region of regions) {
      assert(typeof region === 'string');
      assert(typeof ec2[region] === 'object');
    }
    ...
    this.iterator = new Iterate({
      ...
      handler: async(watchdog) => {
        try {
          ...
          log.error({err}, 'polling spot requets');
          //monitor.reportError(err, 'Error polling spot requests');
        }
      },
    });
  };
  ...
  async _poll(region) {
    ...
    for (let i = 0 ; i < nBatch ; i++) {
      ...
      let result = await this.runaws(this.ec2[region], 'describeSpotInstanceRequests', {
      ...
      if (idsToKill.length > 0) {
        const killed = await this.runaws(this.ec2[region], 'cancelSpotInstanceRequests', {
